...
@Component({
  ...
  template: `
    ...
    <button class="tip-control-button" type="button" (click)="triggerColorPicker()" #button>
      ...
      <i *ngIf="ref.childNodes.length === 0" class="material-icons">format_color_fill</i>
    </button>
  `,
  changeDetection: ChangeDetectionStrategy.OnPush,
  providers: [{provide: BaseControl, useExisting: forwardRef(() => BgColorComponent)}],
})
export class BgColorComponent extends ButtonBaseControl implements OnDestroy {
  ...
  public changeColor(color: string): void {
    this.editor && this.editor.chain().focus().setBgColor(color).run();
  }
